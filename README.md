#  Mohanariprasath's Portfolio

<div align="center">
  <img src="Assests/mj-logo.png" alt="Logo" width="100"/>
  <br/>
  <p><strong>A bleeding-edge, interactive full-stack developer portfolio.</strong><p>

  [![Website Status](https://img.shields.io/website?url=https%3A%2F%2Fshowroom-topaz.vercel.app%2F)](https://showroom-topaz.vercel.app/)
  [![Maintenance](https://img.shields.io/badge/Maintained%3F-yes-green.svg)](https://github.com/Mohanariprasath/Showroom/graphs/commit-activity)
</div>
---

##  Features
- **Dynamic 3D Interactions**: Magnetic navigation links and 3D tilt effects on project cards.
- **Interactive Backgrounds**: Grainy noise filter overlays and a custom HTML5 canvas particle network background.
- **Engaging UI Elements**: Custom smart cursor with hover states and dynamic typewriter hero text.
- **Interactive Skills Orbit**: Custom-built animated HTML5 `<canvas>` showcasing my technology stack orbiting a core center.
- **Scroll Animations**: Smooth intersection-observer based reveal animations and statistical counter effects triggered as you scroll.
- **Responsive Layout**: Fluidly adapts to all viewport sizes, from ultrawide monitors to mobile devices.

---

##  Technologies Used
This portfolio is built with a focus on core web technologies for maximum performance without heavy framework overhead:

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Vercel](https://img.shields.io/badge/vercel-%23000000.svg?style=for-the-badge&logo=vercel&logoColor=white)

---

##  Project Structure

```text
Showroom/
├── Assests/
│   ├── feedtrack.png           # Project #1 Banner Image
│   ├── finance_pro.png         # Project #3 Banner Image
│   ├── icon.png                # Standard Favicon
│   ├── mj-logo.png             # Custom Favicon logo
│   └── Resume Screening.png    # Project #2 Banner Image
├── .gitignore                  
├── README.md                   # Project Documentation
└── index.html                  # Main application & styling
```

---

##  Installation and Setup

To run this project locally on your machine:

1. **Clone the repository**
   ```bash
   git clone https://github.com/Mohanariprasath/Showroom.git
   ```

2. **Navigate into the project directory**
   ```bash
   cd Showroom
   ```

3. **Start a local development server**
   You can use extensions like VS Code's "Live Server" or a simple npx server:
   ```bash
   npx serve ./
   ```

4. **View the application**
   Open your browser and navigate to `http://localhost:3000` (or the port specified by your server).

---

##  Screenshots

<details>
<summary><b>Click to expand screenshots</b> </summary>

###  Hero Interface
*(Upload a screenshot of your hero section here)*

###  Deployments & Projects
*(Upload a screenshot of your project cards here)*

###  Technology Orbit
*(Upload a screenshot of your animated skills orbit here)*

</details>

---

##  License
This project is open-source and available under the standard **MIT License**.
